	...
	public function prepare()
	{
		...
		$this->files[]=new CCodeFile(
			...
			$this->render($controllerTemplateFile)
                ...
                $this->files[] = new CCodeFile(
                        Yii::getPathOfAlias('webroot' . '.' . 'app/model/' . $this->modelClass) . '.js', $this->render($templatePath . DIRECTORY_SEPARATOR . 'app/model/model.php')
                );
                
                $this->files[] = new CCodeFile(
                        Yii::getPathOfAlias('webroot' . '.' . 'app/controller/' . $this->modelClass) . 'Controller.js', $this->render($templatePath . DIRECTORY_SEPARATOR . 'app/controller/controller.php')
                );
                
                $this->files[] = new CCodeFile(
                        Yii::getPathOfAlias('webroot' . '.' . 'app/store/' . $this->modelClass) . '.js', $this->render($templatePath . DIRECTORY_SEPARATOR . 'app/store/store.php')
                );
                
		$files=scandir($templatePath);
		foreach($files as $file)
		{
			if(is_file($templatePath.'/app/view/'.$file) && CFileHelper::getExtension($file)==='php' && $file!=='controller.php')
			{
				$this->files[]=new CCodeFile(
                                        Yii::getPathOfAlias('webroot' . '.' . 'app/view/' . $this->modelClass . '/' . str_replace('.php', '', $file)) . '.js', 
					$this->render($templatePath.'/app/view/'.$file)
				);
			}
		}
	}
